...
class Segments {
    ...
    public function addPoint($x,$y) {
        ...
        $this->prevx = $tx;
        $this->prevy = $ty;
        $this->prevl = $source_l + $l;

        $x = ($tx>$source_x) ? $source_x : $tx;
        $y = ($ty>$source_y) ? $source_y : $ty;
        $m = ($tx<$source_x) ? $source_x : $tx;
        $n = ($ty<$source_y) ? $source_y : $ty;
        
        $source_o = ($x==$m) ? 1 : 0;
        
        $this->segments[$this->segcount] = array('x'=>$x,'y'=>$y,'m'=>($m),'n'=>$n, 'o'=>$source_o, 'p'=>$source_p, 'len'=>$l ,'lent'=>($source_l+$l),'tx'=>$tx,'ty'=>$ty);
        $this->segcount++;
    }
    public function dump() {
        for ($i=0;$i<$this->segcount;$i++) {
            echo json_encode($this->segments[$i])."\n";
        }
    }

    public function import($text) {
        $dir = array( 'L' => array(-1,0), 'R' => array(1,0), 'U' => array(0,-1), 'D' => array(0,1) );
        $data = explode(',',$text);
        foreach ($data as $index => $value) {
            $temp = trim($value);
            $direction = strtoupper(substr($value,0,1));
            $amount = intval(substr($value,1));
            $this->addPoint( $dir[$direction][0] * $amount, $dir[$direction][1] * $amount);
        }
    }
...
function intersect($a,$b) {
    ...
    if ($a['o']==0) { // first segment is horizontal
        if (($b['x']>=$a['x']) && ($b['x']<=$a['m']) && ($b['y']<=$a['y']) && ($b['n']>=$a['y'])) return array($b['x'],$a['y']);
    }
    if ($a['o']==1) { // first segment is vertical
        if (($b['y']>=$a['y']) && ($b['y']<=$a['n']) && ($b['x']<=$a['x']) && ($b['m']>=$a['x'])) return array($a['x'],$b['y']);
    }
    return false;
}

function length2point($text,$point) {
    $dir = array( 'L' => array(-1,0), 'R' => array(1,0), 'U' => array(0,-1), 'D' => array(0,1) );
    $data = explode(',',$text);
    $x = 0;
    $y = 0;
...
for ($i=0;$i<$count1;$i++) {
    ...
    for ($j=0;$j<$count2;$j++) {
        ...
        if ($l1['o']!=$l2['o']) {
            ...
            if ($result!=false) {
                //echo json_encode($l1).' : '.json_encode($l2).' = '.json_encode($result)."\n";
                $value = abs($result[0]) + abs($result[1]);
                if (($value<$min) && ($value>0)) { $min = $value; $val = $result; }
            }
        }
    }
}
echo 'Result for part 1: ';
var_dump($val);
...
for ($i=0;$i<$count1;$i++) {
    ...
    for ($j=0;$j<$count2;$j++) {
        ...
        if ($l1['o']!=$l2['o']) {
            $result = intersect($l1,$l2);
            if (($result[0]==0) && ($result[1]==0)) $result=false;
            if ($result!=false) {
                //echo json_encode($l1)."\n";
                //echo json_encode($l2)."\n";
                $steps1 = length2point($data01,$result);
                $steps2 = length2point($data02,$result);
                $steps = $steps1 + $steps2;
                echo "result x = ".$result[0].', y = '.$result[1]." steps = $steps1 + $steps2 = $steps\n";
                if (($steps<$min) && ($steps>0)) { $min = $steps; $val = $result; }
            }
        }
    }
}
