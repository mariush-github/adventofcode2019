...
class Segments {
    ...
    public function addPoint($x,$y) {
        ...
        $this->segments[$this->segcount] = array('x'=>$x,'y'=>$y,'m'=>($m),'n'=>$n, 'o'=>$source_o);
        $this->segcount++;
    }
...
for ($i=0;$i<2;$i++) {
    ...
    $lines[$i]->import($data[$i]);
    $counter[$i] = $lines[$i]->count();
}
...
for ($i=0;$i<$counter[0];$i++) {
    ...
    for ($j=0;$j<$counter[1];$j++) {
        ...
        if ($l1['o']!=$l2['o']) {
            ...
            if ($result!=false) {
                $steps1 = length2point($data[0],$result);
                $steps2 = length2point($data[1],$result);
                $steps = $steps1 + $steps2;
