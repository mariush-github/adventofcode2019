...
function importData($text) {
	...
	foreach ($data as $idx => $value) {
        ...
        if ($direction == 'L') $x = $x - $amount;
        if ($direction == 'R') $x = $x + $amount;
        if ($direction == 'U') $y = $y - $amount;
        if ($direction == 'D') $y = $y + $amount;
        $records[$count] = array($x,$y);
        $count++;
	}
	return $records;
}
...
//$data01 = 'R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51';
//$data02 = 'U98,R91,D20,R16,D67,R40,U7,R15,U6,R7';
// Part 1 :

$line01 = importData($data01);
$line02 = importData($data02);

$mix = array();
$count = 0;


// go through the segments of first line
for ($i=1;$i<count($line01);$i++) {
    $o1 = 0; // assume horizontal line
    $x11 = $line01[$i-1][0];
    $y11 = $line01[$i-1][1];
    $x12 = $line01[$i][0];
    $y12 = $line01[$i][1];
    $ix = 0;
    $iy = 0;
    if ($x11==$x12) $o1 = 1; // no, it's vertical

    for ($j=1;$j<count($line02);$j++) {
        $o2 = 0; // assume horizontal line
        $x21 = $line02[$j-1][0];
        $y21 = $line02[$j-1][1];
        $x22 = $line02[$j][0];
        $y22 = $line02[$j][1];
        if ($x21==$x22) $o2 = 1; // no, it's vertical

        if ($o1!=$o2) { // lines may intersect
            $intersect = false;
            //2nd line cuts the horizontal line 
            if (($o1==0) && ($x21>=$x11)&& ($x22<=$x12) && ($y21<=$y11) && ($y22>=$y11)) {
                $intersect = true;
                $ix = $x21;
                $iy = $y11;
            }
            //2nd line cuts the vertical line 
            if (($o1==1) && ($x21<=$x11) && ($x22>=$x11) && ($y21>=$y11) && ($y21<=$y12)) {
                $intersect = true;
                $ix = $x11;
                $iy = $y21;
            }
            if ($intersect==true) {
                echo "($x11,$y11)-($x12,$y12) intersects ($x21,$y21)-($x22,$y22) at point ($ix,$iy)\n";
                $mix[count($mix)] = array($ix,$iy);
            }
        }
    }
}
echo "There are ".count($mix)." points where lines intersect.\n";
$min = 99999999999;
$ix=0;
$iy=0;
for ($i=0;$i<count($mix);$i++) {
    $temp = abs($mix[$i][0]) + abs($mix[$i][1]);
    $mix[$i][2] = $temp;
    if ($temp<=$min) {
        $min = $temp;
        $ix = $mix[$i][0];
    ...
    echo $mix[$i][0].','.$mix[$i][1].'='.$mix[$i][2]."\n";
}

echo "The closest point is ($ix,$iy) with a distance equal to $min.\n";
?>